...
namespace Sniper.Tests.Integration.Clients
{
    public class RepositoryCommitsClientTests
    {
        public class TestsWithExistingRepositories
        {
            ...
            private const int SniperNetRepositoryId = 7528679;
            private const string SniperNetRepositoryOwner = "Sniper";
            private const string SniperNetRepositorName = "Sniper.net";
            private const int reactiveGitRepositoryId = 22718025;
            private const string reactiveGitRepositoryOwner = "shiftkey";
            private const string reactiveGitRepositorName = "ReactiveGit";
            ...
            {
                var compareResult = await _fixture.Compare(SniperNetRepositoryOwner, SniperNetRepositorName, "65a22f4d2cff94a286ac3e96440c810c5509196f", "65a22f4d2cff94a286ac3e96440c810c5509196f");
            ...
            {
                var compareResult = await _fixture.Compare(SniperNetRepositoryId, "65a22f4d2cff94a286ac3e96440c810c5509196f", "65a22f4d2cff94a286ac3e96440c810c5509196f");
                Assert.NotNull(compareResult.MergeBaseCommit);
            }
            ...
            {
                var commit = await _fixture.Get(SniperNetRepositoryId, "65a22f4d2cff94a286ac3e96440c810c5509196f");
                Assert.NotNull(commit);
            }
            ...
            public async Task CanGetListOfCommitsBySha()
            {
                ...
                var list = await _fixture.GetAll(SniperNetRepositoryOwner, SniperNetRepositorName, request);
                Assert.NotEmpty(list);
            }
            ...
            {
                var request = new CommitRequest { Path = "Sniper.Reactive/IObservableGitHubClient.cs" };
                var list = await _fixture.GetAll(SniperNetRepositoryOwner, SniperNetRepositorName, request);
                Assert.NotEmpty(list);
            }
            ...
            {
                var commit = await _fixture.Get(SniperNetRepositoryOwner, SniperNetRepositorName, "997e955f38eb0c2c36e55b1588455fa857951dbf");
            ...
            {
                var commit = await _fixture.Get(SniperNetRepositoryId, "997e955f38eb0c2c36e55b1588455fa857951dbf");
            ...
            {
                var sha1 = await _fixture.GetSha1(SniperNetRepositoryOwner, SniperNetRepositorName, "master");
            ...
            {
                var sha1 = await _fixture.GetSha1(SniperNetRepositoryId, "master");
        ...
        public class TestsWithNewRepository : IDisposable
        {
            ...
            private async Task<Reference> CreateTheWorld()
            {
                var master = await _github.Git.Reference.Get(Helper.UserName, _context.RepositoryName, "heads/master");
            ...
            private async Task<TreeResponse> CreateTree(IDictionary<string, string> treeContents)
            {
            ...
            private async Task<Commit> CreateCommit(string message, string sha, string parent)
            {
