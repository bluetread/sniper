...
namespace Sniper.Tests.Integration
{
    public static class EnterpriseHelper
    {
        ...
        {
            var githubUsername = Environment.GetEnvironmentVariable("Sniper_GHE_USERNAME");
            UserName = githubUsername;
            Organization = Environment.GetEnvironmentVariable("Sniper_GHE_ORGANIZATION");

            var githubToken = Environment.GetEnvironmentVariable("Sniper_GHE_OAUTHTOKEN");

            if (githubToken != null)
                return new Credentials(githubToken);

            var githubPassword = Environment.GetEnvironmentVariable("Sniper_GHE_PASSWORD");
        ...
        private static readonly Lazy<bool> _gitHubEnterpriseEnabled = new Lazy<bool>(() =>
        {
            string enabled = Environment.GetEnvironmentVariable("Sniper_GHE_ENABLED");
            return !String.IsNullOrWhiteSpace(enabled);
        });

        private static readonly Lazy<Uri> _gitHubEnterpriseUrl = new Lazy<Uri>(() =>
        {
            string uri = Environment.GetEnvironmentVariable("Sniper_GHE_URL");
        ...
        public static bool IsUsingToken
        {
            get
            {
                return !String.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("Sniper_GHE_OAUTHTOKEN"));
            }
        }

        public static string ClientId
        {
            get { return Environment.GetEnvironmentVariable("Sniper_GHE_CLIENTID"); }
        }

        public static string ClientSecret
        {
            get { return Environment.GetEnvironmentVariable("Sniper_GHE_CLIENTSECRET"); }
        }
        ...
        {
            return new GitHubClient(new ProductHeaderValue("SniperEnterpriseTests"), GitHubEnterpriseUrl)
            {
