...
namespace Sniper.Enterprise
{
    ...
    {
        private static readonly Uri _endPoint = new Uri("/site/sha", UriKind.Relative);
        private readonly ProductHeaderValue _productHeader;
        ...
        public EnterpriseProbe(ProductHeaderValue productInformation, IHttpClient httpClient)
        {
            ...
            _productHeader = productInformation;
        ...
        public async Task<EnterpriseProbeResult> Probe(Uri enterpriseBaseUrl)
        {
            ...
            request.Headers.Add("User-Agent", _productHeader.ToString());

            IResponse response;
            try
            {
                response = await _httpClient.Send(request).ConfigureAwait(false);
            }
